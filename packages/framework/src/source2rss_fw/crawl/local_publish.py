...
from source2rss_fw.scraper import LocateInfo, Sequence, WebsiteScraper
from source2rss_fw.scraper.scraper_error import FailtoGet
...
async def goto_uniform_flow(data, instance: WebsiteScraper, scraper: ScraperNameAndParams, local_timezone: ZoneInfo) -> str:
    """让抓取器运行一次，把数据保存和转换"""
    ...
    if result:
        ...
        flags: LocateInfo = {"amount": scraper.amount} # type: ignore
        sequence = Sequence.PREFER_NEW2OLD

    got_new = [False]
    await save_articles(data, source_name, instance.get(flags, sequence), got_new, scraper.max_rss_item)

    if got_new[0] or data.rss_cache.rss_is_absent(source_name):
        # 当有新内容或文件缺失的情况下，会生成 RSS 并保存
        result = data.db_intf.get_top_n_articles_by_key(source_name, scraper.max_rss_item, key4sort)
        rss_feed = generate_rss(source_info, result, local_timezone)
